...
def parse_decode(s, c):
    m = re.match('([01X]+)\s([123X])\s([0-5X])\s(.*)', s)
    ones = m.group(1).replace('X', '0')
    zeroes = m.group(1).replace('1', 'X').replace('0', '1').replace('X', '0')
    return '        C(0b'+ones+', 0b'+zeroes+', '+m.group(2).replace('X', '0')+', '+m.group(3).replace('X','9')+'), // '+str(c).zfill(3)+': '+m.group(4)

decode_rom = """100XX1XX 3 X STY
...
if __name__ == '__main__':
    counter = 2
    for line in decode_rom.split('\n'):
        print(parse_decode(line, counter))
        counter += 1
